...
FILE = "/storage/emulated/0/Download/端口映射.txt"
# 开始监听 10000-10004 端口
...
# 记录 IP 地址到文件
[ -n "$ipv4_address" ] && echo "本机IPv4地址: $ipv4_address" >> $FILE
[ -n "$ipv6_address" ] && echo "本机IPv6地址: $ipv6_address" >> $FILE
echo "端口映射及占用情况: " >> $FILE
...
for i in $(seq 0 4); do 
    ...
    if [ -n "$process" ]; then
        if [ "$process" = "busybox" ]; then
            echo "$base_port → 公网端口 $actual_port 未被占用" >> $FILE
            # 杀掉所有 busybox 进程
            echo "正在杀掉所有 busybox 进程..."
            killall busybox
        else
            echo "$base_port → 公网端口 $actual_port 被进程 $process 占用" >> $FILE
        fi
    else
        echo "$base_port → 公网端口 $actual_port 未被占用" >> $FILE
    fi
